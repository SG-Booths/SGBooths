...
export default function FollowingScreen({ route, navigation }: any) {
  ...
  // TODO: only one card is starred at a time
          ...
          .filter((obj: any) => {
            return (
              obj.name
                ...
                .replace(/\s{2,}/g, ' ')
                .toLowerCase()
                .includes(search) && vendorsFollowing.includes(obj.uid)
            );
          })
  ...
  const updateStarred = (uid: string, following: boolean) => {
    ...
    if (vendorsFollowing && vendorsFollowing.length > 0) {
      ...
      if (following && starredFilter === true) {
        ...
        let newArray = [...vendorsFollowing, uid];
        newArray && setVendorsFollowing(newArray.sort((a: any, b: any) => b.referrals - a.referrals));
        getStarred(starredFilter);
      }
    } else {
      ...
      setFilteredVendorArray({ ...filteredVendorArray, uid }.sort((a: any, b: any) => b.referrals - a.referrals));
      let newArray = [...vendorsFollowing, uid];
      newArray && setVendorsFollowing(newArray.sort((a: any, b: any) => b.referrals - a.referrals));
      getStarred(starredFilter);
    }
  ...
  const loadNewData = () => {
    ...
    onValue(ref(db, '/users'), (querySnapShot) => {
      let data = querySnapShot.val() || {};
      let vendorList = { ...data };
      let newArray: any = Object.values(vendorList)
        .filter((a: any) => {
  ...
  // TODO: fix: bookmarks take long time to update
  ...
  const searchVendors = (text: string) => {
    ...
    if (text === '') {
      setFilteredVendorArray(vendorArray.sort((a: any, b: any) => b.referrals - a.referrals));
      setNewRender(!newRender);
    }
  };
            ...
            >
              ...
              {!blocked && (
                <TouchableOpacity
                  style={{ marginRight: 20 }}
                  onPress={() => {
                    updateStarred(vendor.uid, vendorsFollowing.includes(vendor.uid));
                  }}
                >
